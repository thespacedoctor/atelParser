...
sys.path.insert(0, os.path.abspath('../../atelParser/atelParser'))
...
html_theme_options = {
    ...
    'github_repo': 'atelParser',
    'strap_line': "scrape and parse content of ATels posted on The Astronomer's Telegram website, identify individual objects by name and coordinates"
}
...
""" % locals()
link_resolver_url = "https://github.com/thespacedoctor/atelParser/tree/master"

# General information about the project.
now = datetime.now()
now = now.strftime("%Y")
project = u'atelParser'
...
exclude_patterns = ['_build', '_templates',
                    '**__version__.py', '**setup.py', 'api/atelParser.rst']
modindex_common_prefix = ["atelParser."]

# (source start file, target name, title, author, documentclass [howto/manual]).
latex_documents = [
    ('index', 'atelParser.tex', u'atelParser Documentation',
     u'Dave Young', 'manual'),
]
...
def generateAutosummaryIndex():
    ...
    # GET ALL SUBPACKAGES
    allSubpackages = ["atelParser"]
    allSubpackages += findAllSubpackges(
        pathToPackage="atelParser"
    )
